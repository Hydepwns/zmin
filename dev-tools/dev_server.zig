...
const Server = struct {
    ...
    fn handleRequest(self: *Server, connection: std.net.Server.Connection) !void {
        ...
        const request_line = lines.next() orelse {
            self.reporter.report(DevToolError.InvalidRequest, errors.contextWithDetails("dev-server", "parsing HTTP request", "missing request line"));
            return DevToolError.InvalidRequest;
        };

        var parts = std.mem.splitSequence(u8, request_line, " ");
        _ = parts.next() orelse {
            self.reporter.report(DevToolError.InvalidRequest, errors.contextWithDetails("dev-server", "parsing HTTP request", "missing HTTP method"));
            return DevToolError.InvalidRequest;
        }; // method
        const path = parts.next() orelse {
            self.reporter.report(DevToolError.InvalidRequest, errors.contextWithDetails("dev-server", "parsing HTTP request", "missing request path"));
            return DevToolError.InvalidRequest;
        };
    ...
    fn handleMinifyApi(self: *Server, connection: std.net.Server.Connection, request: []const u8) !void {
        ...
        const body_start = std.mem.indexOf(u8, request, "\r\n\r\n") orelse {
            self.reporter.report(DevToolError.InvalidRequest, errors.contextWithDetails("dev-server", "parsing minify API request", "missing request body"));
            return DevToolError.InvalidRequest;
        };
        ...
        const input_start = std.mem.indexOf(u8, body, "\"input\":\"") orelse {
            self.reporter.report(DevToolError.InvalidRequest, errors.contextWithDetails("dev-server", "parsing minify API request", "missing 'input' field"));
            return DevToolError.InvalidRequest;
        };
        const input_end = std.mem.indexOfPos(u8, body, input_start + 9, "\"") orelse {
            self.reporter.report(DevToolError.InvalidRequest, errors.contextWithDetails("dev-server", "parsing minify API request", "malformed 'input' field"));
            return DevToolError.InvalidRequest;
        };
        const input = body[input_start + 9 .. input_end];

        const mode_start = std.mem.indexOf(u8, body, "\"mode\":\"") orelse {
            self.reporter.report(DevToolError.InvalidRequest, errors.contextWithDetails("dev-server", "parsing minify API request", "missing 'mode' field"));
            return DevToolError.InvalidRequest;
        };
        const mode_end = std.mem.indexOfPos(u8, body, mode_start + 8, "\"") orelse {
            self.reporter.report(DevToolError.InvalidRequest, errors.contextWithDetails("dev-server", "parsing minify API request", "malformed 'mode' field"));
            return DevToolError.InvalidRequest;
        };
        ...
        const result = zmin.minify(arena.allocator(), input, mode) catch |err| {
            self.reporter.report(err, errors.contextWithDetails("dev-server", "minifying JSON", "minification process failed"));
            return DevToolError.InternalError;
        };
        ...
        const response = std.fmt.allocPrint(self.allocator, "HTTP/1.1 200 OK\r\n" ++
    ...
    fn handleBenchmarkApi(self: *Server, connection: std.net.Server.Connection, request: []const u8) !void {
        // Parse JSON request body
        const body_start = std.mem.indexOf(u8, request, "\r\n\r\n") orelse {
            self.reporter.report(DevToolError.InvalidRequest, errors.contextWithDetails("dev-server", "parsing benchmark API request", "missing request body"));
            return DevToolError.InvalidRequest;
        };
        const body = request[body_start + 4 ..];

        const input_start = std.mem.indexOf(u8, body, "\"input\":\"") orelse {
            self.reporter.report(DevToolError.InvalidRequest, errors.contextWithDetails("dev-server", "parsing benchmark API request", "missing 'input' field"));
            return DevToolError.InvalidRequest;
        };
        const input_end = std.mem.indexOfPos(u8, body, input_start + 9, "\"") orelse {
            self.reporter.report(DevToolError.InvalidRequest, errors.contextWithDetails("dev-server", "parsing benchmark API request", "malformed 'input' field"));
            return DevToolError.InvalidRequest;
        };
        const input = body[input_start + 9 .. input_end];
        ...
        for (modes, 0..) |mode, i| {
            ...
            const result = zmin.minify(arena.allocator(), input, mode) catch |err| {
                self.reporter.report(err, errors.contextWithDetails("dev-server", "benchmarking JSON minification", "minification process failed"));
                return DevToolError.InternalError;
            };
