...
pub const ErrorSeverity = enum {
    ...
    errors, // Errors requiring fallback
...
pub const CommonErrors = struct {
    pub fn outOfMemory(size: u64) ErrorContext {
        return ErrorContext.init(error.OutOfMemory, .errors, "Memory allocation")
    ...
    pub fn fileNotFound(path: []const u8) ErrorContext {
        return ErrorContext.init(error.FileNotFound, .errors, "File access")
    ...
    pub fn invalidJson(path: []const u8, line: u32, col: u32, reason: []const u8) ErrorContext {
        return ErrorContext.init(error.InvalidJson, .errors, "JSON parsing")
...
test "error context formatting" {
    const ctx = ErrorContext.init(error.OutOfMemory, .errors, "Test operation")
...
test "recovery strategy selection" {
    try std.testing.expectEqual(RecoveryStrategy.fallback_to_eco_mode, RecoveryStrategy.forError(error.OutOfMemory));

    try std.testing.expectEqual(RecoveryStrategy.fallback_to_scalar, RecoveryStrategy.forError(error.SimdNotSupported));
}
