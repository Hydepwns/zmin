...
pub const NumaTopology = struct {
    ...
    pub fn getPreferredNode(self: *const NumaTopology) u32 {
        ...
        return self.getNodeForCpu(@intCast(cpu_id));
    }
    
    /// Check if system has multiple NUMA nodes
    pub fn isNumaSystem(self: *const NumaTopology) bool {
        return self.numa_available and self.node_count > 1;
    }
    
    /// Get memory bandwidth estimate between nodes
    pub fn getInterNodeBandwidth(self: *const NumaTopology, node1: u32, node2: u32) f64 {
        _ = self;
...
fn detectLinux(allocator: std.mem.Allocator) !NumaTopology {
    ...
    for (node_list.items, 0..) |node_id, idx| {
        ...
        const cpu_list_path = try std.fmt.allocPrint(
            allocator,
            "{s}/node{d}/cpulist",
            .{ node_dir, node_id }
        );
        ...
        const meminfo_path = try std.fmt.allocPrint(
            allocator,
            "{s}/node{d}/meminfo",
            .{ node_dir, node_id }
        );
...
fn parseCpuList(cpu_list: []const u8, cpu_to_node: []u32, node_id: u32) u32 {
    var count: u32 = 0;
    var iter = std.mem.tokenize(u8, std.mem.trim(u8, cpu_list, " \n"), ",");
...
fn parseNodeMemory(meminfo: []const u8) u64 {
    var lines = std.mem.tokenize(u8, meminfo, "\n");
...
fn detectWindows(allocator: std.mem.Allocator) !NumaTopology {
    // Windows NUMA API requires GetNumaHighestNodeNumber and GetNumaNodeProcessorMask
    // For now, return empty topology
    // TODO: Implement Windows NUMA detection using GetNumaHighestNodeNumber
    return try NumaTopology.initEmpty(allocator);
}
...
fn getSystemMemory() u64 {
    if (builtin.os.tag == .linux) {
        const meminfo = std.fs.cwd().readFileAlloc(
            ...
            "/proc/meminfo",
            4096
        ) catch return 8 * 1024 * 1024 * 1024; // Default 8GB
    ...
    pub fn generate(topology: *const NumaTopology, workload_size: usize) NumaHints {
        if (!topology.isNumaSystem()) {
            return NumaHints{
                ...
                .threads_per_node = @intCast(std.Thread.getCpuCount() catch 4),
