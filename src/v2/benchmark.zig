...
const StreamingParser = @import("streaming/parser.zig").StreamingParser;
const ParserConfig = @import("streaming/parser.zig").ParserConfig;
const SimdLevel = @import("streaming/parser.zig").SimdLevel;
...
pub fn runBenchmark() !void {
    ...
    const configs = [_]struct { name: []const u8, config: ParserConfig }{
        ...
        .{ .name = "Auto SIMD", .config = ParserConfig{ .enable_simd = true, .simd_level = .auto } },
    };
