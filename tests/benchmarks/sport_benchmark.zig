...
pub fn main() !void {
    ...
    try stdout.print("Cache Efficiency Test (different data sizes):\n", .{});
    const chunk_sizes = [_]usize{ 4096, 16384, 65536, 262144, 1048576 };

    for (chunk_sizes) |chunk_size| {
        // Test sport mode with different data sizes (chunk_size represents test data size)
        const input = try generateTestJson(allocator, chunk_size);
        defer allocator.free(input);
        ...
        try MinifierInterface.minify(allocator, .sport, stream.reader(), output.writer());
        const elapsed = timer.read();

        const throughput = (@as(f64, @floatFromInt(chunk_size)) / @as(f64, @floatFromInt(elapsed))) * 1000.0;
        try stdout.print("  Data size {d}KB: {d:.2} MB/s\n", .{
            chunk_size / 1024,
            throughput,
        });
    }
