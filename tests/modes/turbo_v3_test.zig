// Test for TURBO V3 parallel implementation
const std = @import("std");
const TurboMinifierParallelV3 = @import("turbo_minifier_parallel_v3").TurboMinifierParallelV3;
...
pub fn main() !void {
    ...
    try stdout.print("\nTURBO V3 Parallel Implementation Test\n", .{});
    try stdout.print("=====================================\n\n", .{});
    ...
    for (test_sizes) |size| {
        ...
        const config = TurboMinifierParallelV3.ParallelConfig{
            .thread_count = thread_count,
            .chunk_size = 256 * 1024,
        };
        
        var parallel = try TurboMinifierParallelV3.init(allocator, config);
        ...
        try stdout.print("  Simple:   {d} ms ({d:.2} MB/s)\n", .{
            simple_time,
            if (simple_time > 0) @as(f64, @floatFromInt(size)) / @as(f64, @floatFromInt(simple_time)) / 1024.0 else 0.0
        });
