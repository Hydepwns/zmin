...
fn testContentTypes(allocator: std.mem.Allocator, simd_minifier: *AdvancedSIMDMinifier) !void {
    ...
    const content_types = [_]struct { content: []const u8, name: []const u8 }{
        .{ .content = "    {    \"high_whitespace\"    :    \"lots of spaces and tabs\"\t\t,\n\n    \"more\"   :   \"content\"    }", .name = "High Whitespace" },
        .{ .content = "{\"compact\":\"minimal\",\"spaces\":false,\"efficient\":true}", .name = "Already Compact" },
