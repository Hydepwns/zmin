    ...
    pub fn run(self: *Self, args: []const []const u8) !void {
        ...
        if (std.mem.eql(u8, command, "show")) {
            ...
            if (std.mem.eql(u8, profile_command, "list")) {
                ...
                if (args.len < 3) {
                    self.reporter.report(DevToolError.MissingArgument, errors.contextWithDetails(
                        "config-manager",
                        "profile apply",
                        "Usage: config-manager profile apply <profile_name>"
                    ));
                    return DevToolError.MissingArgument;
                }
                try self.applyProfile(args[2]);
            } else {
                self.reporter.report(DevToolError.UnknownCommand, errors.contextWithDetails(
                    "config-manager",
                    "profile command",
                    profile_command
                ));
            ...
            if (args.len < 3) {
                self.reporter.report(DevToolError.MissingArgument, errors.contextWithDetails(
                    "config-manager",
                    "set command",
                    "Usage: config-manager set <key> <value>"
                ));
                return DevToolError.MissingArgument;
            }
