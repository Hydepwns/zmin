...
const ConfigManager = struct {
    ...
    pub fn init(allocator: std.mem.Allocator) Self {
        return Self{
            .allocator = allocator,
            .reporter = ErrorReporter.init(allocator, "config-manager"),
        };
    }
    ...
    pub fn run(self: *Self, args: []const []const u8) !void {
        ...
        if (std.mem.eql(u8, command, "show")) {
            ...
            if (std.mem.eql(u8, profile_command, "list")) {
                ...
                if (args.len < 3) {
                    self.reporter.report(DevToolError.MissingArgument, errors.contextWithDetails("config-manager", "profile apply", "Usage: config-manager profile apply <profile_name>"));
                    return DevToolError.MissingArgument;
                }
                try self.applyProfile(args[2]);
            } else {
                self.reporter.report(DevToolError.UnknownCommand, errors.contextWithDetails("config-manager", "profile command", profile_command));
                try self.showProfileHelp();
                return DevToolError.UnknownCommand;
            }
        } else if (std.mem.eql(u8, command, "set")) {
            if (args.len < 3) {
                self.reporter.report(DevToolError.MissingArgument, errors.contextWithDetails("config-manager", "set command", "Usage: config-manager set <key> <value>"));
                return DevToolError.MissingArgument;
            }
            try self.setConfigValue(args[1], args[2]);
        } else if (std.mem.eql(u8, command, "get")) {
            if (args.len < 2) {
                self.reporter.report(DevToolError.MissingArgument, errors.contextWithDetails("config-manager", "get command", "Usage: config-manager get <key>"));
                return DevToolError.MissingArgument;
            }
            try self.getConfigValue(args[1]);
        } else {
            self.reporter.report(DevToolError.UnknownCommand, errors.contextWithDetails("config-manager", "parsing command", command));
            try self.showHelp();
            return DevToolError.UnknownCommand;
        }
    }
    ...
    fn showConfig(self: *Self) !void {
        ...
        const config_content = file_ops.readFile(self.allocator, "config/zmin.toml") catch |err| {
    ...
    fn validateConfig(self: *Self) !void {
        ...
        if (error_count == 0) {
            ...
            std.log.err("❌ Configuration validation failed with {d} errors", .{error_count});
        }
    }
    ...
    fn listProfiles(self: *Self) !void {
        ...
        var preset_dir = std.fs.cwd().openDir("config/presets", .{ .iterate = true }) catch |err| {
            if (err == error.FileNotFound) {
        ...
        while (try iterator.next()) |entry| {
            if (entry.kind == .file and std.mem.endsWith(u8, entry.name, ".toml")) {
                ...
                var lines = std.mem.splitScalar(u8, profile_content, '\n');
    ...
    fn setConfigValue(self: *Self, key: []const u8, value: []const u8) !void {
        _ = self; // TODO: Use self when implementing actual TOML modification
