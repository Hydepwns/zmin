...
const Debugger = struct {
    ...
    pub fn debugMinify(self: *Debugger, input: []const u8, mode: zmin.ProcessingMode) ![]const u8 {
        ...
        const result = zmin.minify(self.allocator, input, mode) catch |err| {
            self.reporter.report(err, errors.contextWithDetails("debugger", "minifying JSON", "debug minification failed"));
            return DevToolError.InternalError;
        };
    ...
    pub fn analyzePerformance(self: *Debugger, input: []const u8) !void {
        ...
        for (modes, 0..) |mode, i| {
            ...
            const result = zmin.minify(self.allocator, input, mode) catch |err| {
                self.reporter.report(err, errors.contextWithDetails("debugger", "performance analysis", "minification failed"));
                return DevToolError.InternalError;
            };
    ...
    pub fn benchmark(self: *Debugger, input: []const u8, iterations: u32) !void {
        ...
        for (modes) |mode| {
            ...
            for (0..iterations) |i| {
                ...
                const result = zmin.minify(self.allocator, input, mode) catch |err| {
                    self.reporter.report(err, errors.contextWithDetails("debugger", "benchmark minification", "minification failed"));
                    return DevToolError.InternalError;
                };
                const elapsed = timer.read();
    ...
    pub fn memoryStressTest(self: *Debugger, base_size: usize, multiplier: u32) !void {
        ...
        for (0..multiplier) |i| {
            ...
            const result = zmin.minify(arena.allocator(), test_data, .sport) catch |err| {
                self.reporter.report(err, errors.contextWithDetails("debugger", "memory stress test", "minification failed"));
                return DevToolError.InternalError;
            };
            ...
            self.log(.basic, "  Size {d}: {d:.3}ms, {d} bytes memory, {d:.2}% compression", .{ test_data.len, @as(f64, @floatFromInt(elapsed)) / 1_000_000.0, mem_used, (@as(f64, @floatFromInt(result.len)) / @as(f64, @floatFromInt(test_data.len))) * 100.0 });
