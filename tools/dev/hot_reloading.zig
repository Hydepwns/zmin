...
const HotReloader = struct {
    ...
    fn printStats(self: *HotReloader) void {
        ...
        std.log.info("📊 Stats: {d} builds, {d:.1}% success, avg {d:.1}ms", .{ self.stats.total_builds, success_rate, self.stats.average_build_time });
    }
...
pub fn main() !void {
    ...
    while (i < args.len) : (i += 1) {
        ...
        if (std.mem.eql(u8, arg, "--build-command") or std.mem.eql(u8, arg, "-b")) {
            if (i + 1 >= args.len) {
                hot_reloader.reporter.report(DevToolError.MissingArgument, errors.contextWithDetails("hot-reloading", "parsing arguments", "Missing build command after -b/--build-command"));
            ...
            if (i + 1 >= args.len) {
                hot_reloader.reporter.report(DevToolError.MissingArgument, errors.contextWithDetails("hot-reloading", "parsing arguments", "Missing debounce time after -d/--debounce"));
                return DevToolError.MissingArgument;
            }
            const debounce_ms = std.fmt.parseInt(u64, args[i + 1], 10) catch {
                hot_reloader.reporter.report(DevToolError.InvalidArguments, errors.contextWithDetails("hot-reloading", "parsing debounce time", args[i + 1]));
            ...
            hot_reloader.reporter.report(DevToolError.UnknownCommand, errors.contextWithDetails("hot-reloading", "parsing arguments", arg));
            printHelp();
