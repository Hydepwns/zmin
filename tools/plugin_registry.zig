...
const PluginRegistry = struct {
        ...
        if (std.mem.eql(u8, command, "list")) {
            ...
            if (args.len < 2) {
                self.reporter.report(DevToolError.MissingArgument, errors.contextWithDetails(
                    "plugin-registry", "processing info command", "missing plugin index argument"
                ));
            ...
            const index = std.fmt.parseInt(usize, args[1], 10) catch {
                self.reporter.report(DevToolError.InvalidArguments, errors.contextWithDetails(
                    "plugin-registry", "parsing plugin index", args[1]
                ));
            ...
            self.reporter.report(DevToolError.UnknownCommand, errors.contextWithDetails(
                "plugin-registry", "processing command", command
            ));
            try self.showHelp();
        ...
        for (self.loader.plugins.items, 0..) |plugin, i| {
            ...
            const info = self.loader.getPluginInfo(i) catch |err| {
                self.reporter.report(err, errors.contextWithDetails(
                    "plugin-registry", "getting plugin info for testing", "plugin index out of range"
                ));
                continue;
            };
            ...
            const result = self.loader.processWithPlugin(i, test_json) catch |err| {
                self.reporter.report(err, errors.contextWithDetails(
                    "plugin-registry", "testing plugin", info.name
                ));
    ...
    fn showPluginInfo(self: *Self, index: usize) !void {
        ...
        const info = self.loader.getPluginInfo(index) catch |err| {
            self.reporter.report(err, errors.contextWithDetails(
                "plugin-registry", "getting plugin info", "invalid plugin index"
            ));
    ...
    fn benchmarkPlugins(self: *Self) !void {
        ...
        for (test_cases) |test_case| {
            ...
            for (self.loader.plugins.items, 0..) |plugin, i| {
                ...
                const info = self.loader.getPluginInfo(i) catch |err| {
                    self.reporter.report(err, errors.contextWithDetails(
                        "plugin-registry", "getting plugin info for benchmarking", "invalid plugin index"
                    ));
                    continue;
                };
